...
class App{
  ...
  renderVals(){
    ...
    return {
      ...
      qImg:q?q.img.u:'',qW:q&&q.img.w?q.img.w:'',qH:q&&q.img.h?q.img.h:'',qAspect:q?(q.img.p?'Planche':(q.img.a.map(a=>this.label(this.ASP,a)).join(' · ')||'Divers')):'',
      ...
      fImgAsp:fcur?(fcur.p?'Planche — dessin idéalisé, pas une photo de terrain':('Cette photo montre : '+(fcur.a.map(a=>this.label(this.ASP,a)).join(', ')||'divers'))):'',
